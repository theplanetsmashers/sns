...
プロ品質のオンライン講座スライドで一般的な要素(セクションごとの配色・カード風の箇条書き・強調表現など)を取り入れて設計しています(`lib/palette.js` にアクセントカラーの定義)。

- 上端のカラーバー・薄いドットテクスチャ・タイトルスライドの装飾円
...
まだ対応できていない要素(今後の拡張候補):

- 図解・スクリーンショット・イラストなどの画像(現状は画像生成機能がないため非対応)
- 2カラムや比較表などのレイアウトバリエーション、コードブロック風の表示
- 内容に応じたアイコンでの視覚分類(Tips/注意/手順など)
- Tips/注意のコールアウトボックス
- スライド遷移のフェードなど動画側の演出(ファイルサイズ増とのトレードオフ)

## 今後の拡張ポイント

- 上記のスライドデザインの拡張候補
- スライドデザインのテーマ(配色・フォント)を選べるようにする
- 章立て(複数動画に分割したコース全体)への対応。対応すればコース全体の中の位置表示(Lecture 3/12等)や前回の振り返り/次回予告スライドも実装しやすくなる
