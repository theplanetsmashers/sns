...
- `lib/generateOutline.js` — Claude APIで講義構成(タイトル・学習目標・スライド内容・ナレーション原稿・アイコン・レイアウト)を生成
- `lib/buildDeck.js` — 構成データをタイトル/本編/まとめの「スライド配列」に平坦化する共通処理
- `lib/palette.js` — セクションごとのアクセントカラー(色ローテーション)の定義
- `lib/icons.js` — アイコンイラストのSVGライブラリ(約25種類)
- `lib/renderIconPng.js` — PPTX埋め込み用に、アイコンをPuppeteerで透過PNGにラスタライズする
- `lib/buildPptx.js` — pptxgenjsで実際の.pptxファイルを生成(list/process/callout/codeの4レイアウトに対応)
- `lib/synthesizeAudio.js` — VOICEVOX(無料)を優先してナレーション音声を生成。失敗時はOpenAI TTS→無音の順にフォールバック
- `lib/renderSlideImages.js` — 各スライドをHTML/CSSで描画し、Puppeteerでスクリーンショット(720p、list/process/callout/codeの4レイアウトに対応)
- `lib/buildVideo.js` — ffmpegでスライド画像+音声をクロスフェードしながら結合し、講義動画(mp4)を組み立てる
- `bot.js` — GitHub Issueをトリガーに上記一式を実行し、Issueコメント+Discordへのファイル添付通知まで行うエントリーポイント
- `state/history.json` — 生成履歴の記録

## スライドデザイン

プロ品質のオンライン講座スライドと比較して洗い出した30の改善項目のうち、実装できるもの(画像生成AIなしで実現できる範囲)はすべて反映しています。

**アイコンイラスト(内容をイメージしやすくする)**

写実的な画像生成AIは使っていません(未契約)が、内容に応じてClaudeが選んだアイコンを、状況に応じたレイアウトで表示することで「見てイメージできる」教材にしています(`lib/icons.js` に約25種類のフラットなラインアイコンを収録)。

- **list(通常)**: 左にテキスト、右に選ばれたアイコンを円形パネルで表示(2カラム構成)
- **process(手順)**: 手順が2〜4ステップの場合、横並びの矢印フロー図で見せる
- **callout(注意・強調)**: 重要な1メッセージを、アイコン付きの大きな強調ボックスで見せる
- **code(コード例)**: プログラミングのコード例をターミナル風の見た目で見せる

内容に応じてClaudeがこの4パターンから自動で選ぶので、箇条書きばかりが続かず、スライドごとに適切な分量・見せ方になります。

**その他のデザイン要素**

- 上端のカラーバー・薄いドットテクスチャ・コンテンツ全体を囲む枠線
- コンテンツスライドごとにアクセントカラーをローテーション(青→ティール→紫→ローズ)。タイトル/まとめは青でブックエンド
- 箇条書きはカード風(薄い背景+左アクセントバー)、バッジには影を付けて立体感を出す
- 「用語:説明」形式の文章は用語部分を自動検出して色付き太字に強調
- タイトルスライドに枚数・想定所要時間を表示
- 動画のスライド間はクロスフェードで滑らかに切り替わる(ffmpegの`xfade`/`acrossfade`)
- PPTX側もアイコン(ラスタライズしたPNGを埋め込み)・4レイアウト・枠線など、動画スライドとほぼ同じデザインに統一

**技術的な制約でそのまま反映できなかった項目**(将来、コース分割機能などを実装すれば対応可能になるもの):

- コース全体の中の位置表示(例: Lecture 3/12)、前回の振り返り・次回予告スライド → 現状は1リクエスト=1レクチャー(単発動画)という設計のため、コース全体の構成を持つ機能がないと実装できません
- 講師・コースロゴのウォーターマーク → 汎用ツールのため、ユーザーごとのロゴ素材を持っていません(要望があれば設定できるようにする余地あり)
- 音声の抑揚・感情表現の調整 → 無料音声合成(VOICEVOX)の機能的な制約です

## 今後の拡張ポイント

- 講師・コースロゴなど、ユーザーごとのブランディング要素を設定できるようにする
- 章立て(複数動画に分割したコース全体)への対応。対応すればコース全体の中の位置表示や前回の振り返り/次回予告スライドも実装しやすくなる
...
- 動画のクオリティはスライド+ナレーションの組み合わせです(アニメーションやアバターはありません)。まずは工数削減を優先したv1という位置づけです
