...
- [`note-substack-generator/`](note-substack-generator/) — note記事を貼り付けると、Substack用の英語導入記事をClaude APIで自動生成するローカルWebアプリ。英語圏読者への導線作りを目的としたツール。

いずれのツールも、実際の投稿・送信は人間が最終判断して手動で行う設計です(全自動投稿は行いません)。
