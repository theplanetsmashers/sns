...
2. **SNS下書き工場**(Webアプリ) — note記事を貼り付けると、フックの効いたThreads投稿文・X投稿文・Substack用の英語紹介記事をその場で自動生成し、コピーボタン付きの履歴として並べて表示
...
note記事を貼り付けると、フックの効いたThreads投稿文・X投稿文・Substack用の英語紹介記事をその場で生成するツールです。
...
3. note・Threads・X・Substackの4列が横並びで表示され、生成した下書きはThreads/X/Substack列にコピーボタン付きで並ぶ
4. 縦方向には過去の生成履歴が並ぶので、前回・今回・次回の流れを見返せる
5. note・Threads・X・Substackへの実際の投稿は、それぞれ別のタイミングで手動で行う

Threads/Xは短い日本語のJSON応答、Substackは長い英語のプレーンテキスト応答として、それぞれ別のAI呼び出しに分けて生成しています(1つのJSON応答に長い英文まで詰め込むと、応答が長くなるほど齟齬なく完結せずJSON解析に失敗しやすくなるため)。
