...
- [`manager-1on1-tool/`](manager-1on1-tool/) — 新任管理職向け1on1支援ツール(フェーズ3)。部下プロフィールを登録すると、Claude APIが1on1前のアジェンダ提案・1on1後の要点抽出・複数回の記録からの傾向分析を支援する。GitHub Issueを作るだけでスマホだけで完結する運用と、PCでローカルWebアプリとして使う運用の2通りに対応。
