...
- [`higgsfield-review-generator/`](higgsfield-review-generator/) — Higgsfield(外部サービス)のMarketing Studioを使い、商品の販売ページURLからAIキャラクターが商品を紹介するレビュー動画を作るための手順書とプロンプト集。Higgsfieldに公開APIがないため自動化はできず、手動でブラウザ操作する運用。

どのツールも、実際の投稿・送信は人間が最終判断して手動で行う設計です(全自動投稿は行いません)。
