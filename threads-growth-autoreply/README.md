...
## 費用について(このリポジトリがpublicの場合)

- **GitHub Actions**: publicリポジトリは実行時間が無制限で無料。30分おきの定期実行でも課金は発生しない。
- **Threads API**: 無料。
- **Discord Webhook**: 無料。
- **Claude API**: 唯一の実費。`claude.ai`のサブスクとは別課金で、従量制。返信1件あたり入力500トークン・出力60トークン程度と仮定すると1件あたり1円未満で、通常の運用量なら月あたり数百円程度に収まる見込み。API呼び出しは実際に新しいコメントが届いたときだけ発生するため、待機中(伸びている投稿がない間)のコストはかからない。
...
- **publicリポジトリでの注意**: `state/`配下はワークフローがgitにコミットするため、内容はコミット履歴として誰でも閲覧できる。相手のコメント本文・ユーザー名はコミットされるファイルには保存しない設計にしている(重複返信の防止はコメントIDのみで行い、言い回しの重複回避は自分の生成した返信文のみを参照する)。自分の投稿本文(`state/snapshots.json`)は元々Threads上で公開済みの内容なので問題ない。
