...
const { writeCaptionsFile } = require("./lib/buildCaptions");
const { burnCaptions } = require("./lib/burnCaptions");
...
async function generateTeaser({ noteTitle, articleText, noteUrl, note, vertical = true }) {
  ...
  console.log("[2/7] ナレーション音声を生成中(VOICEVOX優先)...");
  const audioDir = path.join(outDir, "audio");
  const audioResult = await synthesizeAudio(script.scenes, audioDir);
  const durations = await Promise.all(audioResult.audioPaths.map(getDurationSeconds));

  console.log("[3/7] 背景イラストを生成中(OPENAI_API_KEY設定時のみ。未設定ならアイコン装飾にフォールバック)...");
  ...
  const withBgmPath = path.join(outDir, "teaser-with-bgm.mp4");
  await mixBgm(narratedPath, bgmPath, withBgmPath, bgmVolume);

  console.log("[7/7] テロップ(字幕)を焼き込み中...");
  const assPath = path.join(outDir, "captions.ass");
  writeCaptionsFile(script.scenes, durations, dims, assPath);
  const videoPath = path.join(outDir, "teaser.mp4");
  await burnCaptions(withBgmPath, assPath, videoPath);
