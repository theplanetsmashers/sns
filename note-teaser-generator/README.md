...
各シーンは「文字だけの画面」にならないよう、背景に絵(画像)を敷いた上にテロップ(字幕)としてキャプションを乗せる構成にしています。さらに無音にはせず、聴き心地の良いBGMを常に流します。

生成されるもの:

- `script.json` / `script.txt` — シーンごとの表示テキスト(テロップ)・ナレーション原稿・背景イラストの指定・YouTube概要欄案
- `art/art-XX.png` — シーンごとの背景イラスト(`OPENAI_API_KEY`設定時のみ。AI画像生成)
- `images/scene-XX.png` — 背景の絵(AI生成イラスト、なければアイコン装飾)の上にテロップを乗せて描画した各シーンの画像(既定: 縦1080x1920 = YouTube Shorts想定)
- `audio/narration-XX.wav` — 各シーンのナレーション音声
- `bgm.m4a` — 動画の尺に合わせてffmpegでその場合成した、著作権フリーのアンビエントBGM
- `teaser-narration.mp4` — ナレーション入りの動画(BGM合成前の中間ファイル)
- `teaser.mp4` — 上にBGMを重ねた最終的な紹介動画(約30秒)
...
| 背景イラスト | 既定はアイコン+グラデーションの装飾背景(無料・APIキー不要)。`OPENAI_API_KEY`設定時のみOpenAI Images APIで実写風イラストに切り替え | 無料(既定) |
...
事前に用意が必要なのは実質 `ANTHROPIC_API_KEY` だけです。`OPENAI_API_KEY` を設定した場合は、ナレーションがより自然な音声(OpenAI TTS)に切り替わるのに加えて、背景がアイコン装飾からAI生成イラストに切り替わります(どちらも有料・任意)。
...
- `lib/renderTeaserImages.js` — 各シーンをHTML/CSSで描画し、Puppeteerでスクリーンショット(既定: 縦1080x1920)。背景の絵の上に、テロップ(字幕)としてキャプションを乗せる
...
- 動画のクオリティはシーン画像(背景の絵+テロップ)+ナレーション+BGMの組み合わせです(実写映像やアニメーション、アバターはありません)。まずは工数削減を優先したv1という位置づけです
